...
private _refPlayerPos = (_allPlayerPositions select (floor (random (count _allPlayerPositions))));
...
while {!_isOk && _tries < 10} do {
   ...
   if (count _roadSegments > 0) then {
      private _roadSegment = _roadSegments select (floor (random (count _roadSegments)));
